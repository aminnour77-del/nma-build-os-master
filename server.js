...
app.get('/api/tubi', async (req, res) => {
  try {
    ...
    console.error('Errore Geospaziale:', err);
...
app.get('/api/cantieri', async (req, res) => {
  try {
    ...
    res.status(500).send('Errore recupero cantieri');
  }
});
  ...
  res.send(`
    ...
    <head>
        ...
        <style>
            ...
            select { width: 100%; padding: 10px; background: #222; color: #fff; border: 1px solid #444; border-radius: 6px; margin-top: 8px; font-size: 14px; }
...
app.get('/cantiere', (req, res) => {
  res.send(`
    ...
    <body>
        <div class="header">
            <h1>NMA BUILD OS</h1>
            <p style="margin:5px 0 0 0; color:#888; font-size: 14px;">Terminale Scavo Multi-Appalto</p>
        </div>
...
app.listen(PORT, () => { console.log('✅ SERVER MULTI-CANTIERE STABILE ONLINE'); });
